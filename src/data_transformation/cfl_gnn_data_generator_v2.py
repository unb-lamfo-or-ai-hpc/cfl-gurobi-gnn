...
def process_single_instance(lp_file_path: str, output_dir: str, 
    ...
    try:
        ...
        presolved_features = extract_model_features(model_presolved)
        presolved_lp_path = os.path.join(instance_dir, "presolved.lp")
        model_presolved.write(presolved_lp_path)
        ...
        callback = DataCollectionCallback(model_presolved)
        
        model_presolved.setParam('TimeLimit', time_limit)
        
        start_time = time.time()
        model_presolved.optimize(callback)
        ...
        if model_presolved.SolCount > 0:
            for i in range(min(model_presolved.SolCount, model_presolved.Params.PoolSolutions)):
                try:
                    model_presolved.setParam("SolutionNumber", i)
                    pool_obj = model_presolved.PoolObjVal
                    pool_solution = np.array([v.Xn for v in model_presolved.getVars()])
        ...
        final_solution = extract_solution_features(model_presolved, 
                                                   model_presolved.NodeCount,
                                                   solve_time)
        ...
        metadata = {
            'instance': instance_name,
            'status': model_presolved.Status,
            'status_name': {
                1: 'LOADED', 2: 'OPTIMAL', 3: 'INFEASIBLE', 
                4: 'INF_OR_UNBD', 5: 'UNBOUNDED', 9: 'TIME_LIMIT'
            }.get(model_presolved.Status, 'UNKNOWN'),
            'runtime': solve_time,
            'mip_gap': getattr(model_presolved, 'MIPGap', 0.0),
            'node_count': getattr(model_presolved, 'NodeCount', 0),
            'num_solutions_found': model_presolved.SolCount,
        ...
        # Optional: Save dataframes for easy analysis
