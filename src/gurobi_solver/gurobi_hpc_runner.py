"""
Ejecutor Paramétrico de Gurobi para HPC
=======================================
Permite inyectar parámetros críticos (Presolve, Método LP, Heurísticas, MIPFocus) 
y archivos de Warm Start (.mst) directamente desde el entorno Slurm.
"""
...
def process_single_instance(lp_file_path: str, output_dir: str, 
                           env: gp.Env, args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        print(f"  [3/7] Configurando Gurobi HPC (Presolve={args.presolve}, Método={args.method})...")
        model_to_solve = model.copy()
        
        # Inyección de parámetros definidos por el usuario en Slurm
        model_to_solve.setParam('Presolve', args.presolve)
        ...
        if args.warm_start and os.path.exists(args.warm_start):
            print(f"  [+] Inyectando Warm Start desde: {os.path.basename(args.warm_start)}")
            model_to_solve.read(args.warm_start)

        print("  [4/7] Extrayendo características post-Presolve...")
        ...
        print(f"  [5/7] Solving with time limit {args.time_limit}s...")
        ...
        solution_data = {
            ...
            'existing_benchmark_solution': None
        }
...
def main():
    parser = argparse.ArgumentParser(description="Gurobi HPC Parametric Runner")
    ...
    # Nuevos Parámetros Exclusivos para Control Gurobi
    parser.add_argument('--threads', type=int, default=None, help="Número de hilos. Si es None, usa SLURM_CPUS_PER_TASK.")
    parser.add_argument('--presolve', type=int, default=-1, help="-1: Auto, 0: Off, 1: Conservador, 2: Agresivo")
    parser.add_argument('--method', type=int, default=-1, help="-1: Auto, 0: Primal, 1: Dual, 2: Barrera, 3: Concurrente")
    parser.add_argument('--heuristics', type=float, default=0.05, help="Fracción de tiempo dedicado a heurísticas (0.0 a 1.0)")
    parser.add_argument('--mipfocus', type=int, default=0, help="0: Equilibrado, 1: Factibilidad, 2: Optimalidad, 3: Cota")
    parser.add_argument('--warm_start', type=str, default=None, help="Ruta al archivo .mst generado por la GNN")
    
    args = parser.parse_args()
    
    print("=" * 60)
    print("Gurobi HPC Parametric Runner")
    print("=" * 60)
    ...
    for i, (category, file_path) in enumerate(file_paths, 1):
        ...
        success = process_single_instance(file_path, category_out_dir, env, args)
